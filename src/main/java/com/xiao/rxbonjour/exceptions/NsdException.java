package com.xiao.rxbonjour.exceptions;
...
public class NsdException extends IllegalStateException{
    ...
    private String buildMessageFrom(@FAILURE String failure,
        ...
        if (failure.equals(INVALID_PROTOCOL)) {
            ...
            return String.format(Locale.getDefault(), "You shall not use the constant RxBonjour.ALL_AVAILABLE_SERVICES or the " +
                    "value \"_services._dns-sd._udp\" with the RxBonjour.startDiscovery(Context, String) method. This value will " +
                    "not allow the services to be resolved. If you need to list all the available services, use the RxBonjour.startDiscovery(Context) " +
                    "or provide a valid protocol to the RxBonjour.startDiscovery(Context, String) call.");
        } else {
            return String.format(Locale.getDefault(),
                    "RxBonjour failed %s on service %s with error code %d - ", failure, serviceName, errorCode);
        }
